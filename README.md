...
- **Отображение с адресами:** в интерфейсе данные отображаются в формате `[0x0100]=0x0007` (hex) и `[0x0100]=7` (dec)

**Алгоритм:**
1. Инициализация: R0 = 0 (аккумулятор), R1 = 1 (индекс)
2. Загрузка размера массива из памяти `[0x0100]`
3. Цикл: пока индекс <= размер
   - Вычисление адреса элемента: базовый адрес (0x0100) + индекс
   - Загрузка элемента в регистр через косвенно-регистровую адресацию
   - Добавление к сумме
   - Увеличение индекса
4. Результат в R0
...
**Ожидаемый результат:** `50` (в регистре R0)
...
3. Цикл: для i от 1 до размера
   - Вычисление адреса A[i]: 0x0200 + индекс
   - Загрузка A[i] из памяти через косвенно-регистровую адресацию
   - Вычисление адреса B[i]: 0x0300 + индекс
   - Загрузка B[i] из памяти через косвенно-регистровую адресацию
   - Умножение A[i] * B[i]
...
- **Отображение с адресами:** данные отображаются в формате `[0x0100]=0x0007` (hex) и `[0x0100]=7` (dec)
- **Для задачи 1:** отображение массива данных (0x0100-0x0107) и значения аккумулятора R0
...
#### Версия 2.0.0 (2024)
- ✅ Реализована архитектура фон Неймана для задачи 2 (команды хранятся в RAM)
- ✅ Добавлена постепенная загрузка данных для задач 1 и 2
- ✅ Улучшено отображение данных в MemoryView с адресами (формат `[0x0100]=0x0007`)
