...
; Массив: [10, 20, 30, 40, 50, 60, 70] (7 элементов)
; Ожидаемый результат: 280

; Инициализация
LDI R0, 0          ; R0 = 0 (аккумулятор)
LDI R1, 7          ; R1 = 7 (размер массива)
LDI R2, 0          ; R2 = 0 (индекс)

; Загрузка элементов массива в память
LDI R3, 10
STR R3, [0x1000]   ; [0x1000] = 10
LDI R3, 20
STR R3, [0x1001]   ; [0x1001] = 20
LDI R3, 30
STR R3, [0x1002]   ; [0x1002] = 30
LDI R3, 40
STR R3, [0x1003]   ; [0x1003] = 40
LDI R3, 50
STR R3, [0x1004]   ; [0x1004] = 50
LDI R3, 60
STR R3, [0x1005]   ; [0x1005] = 60
LDI R3, 70
STR R3, [0x1006]   ; [0x1006] = 70

; Основной цикл
LOOP_START:
CMP R2, R1         ; Сравнить индекс с размером
JZ LOOP_END        ; Если индекс == размер, выйти из цикла

; Загрузить элемент массива
ADD R3, R2, 0x1000 ; R3 = R2 + 0x1000 (адрес элемента)
LDRR R4, [R3]      ; R4 = [R3] (значение элемента)

; Добавить к сумме
ADD R0, R0, R4     ; R0 = R0 + R4 (сумма)
...
HALT
    ...
    def setup_task_data(self, processor: RISCProcessor, task_id: int):
        ...
        if task_id == 1:  # Сумма массива
            ...
            # Загружаем элементы массива в память начиная с 0x1000
            for i, value in enumerate(elements):
                processor.memory.ram[0x1000 + i] = value
                print(f"Stored element[{i}] = {value} at address 0x{0x1000 + i:04X}")
